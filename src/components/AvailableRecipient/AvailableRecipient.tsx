...
const AvailableRecipient: FC = (): ReactElement => {
  const { data, onDispatch, onEnableAlertDialog, onEnableToast } =
    useLayoutContext();
  ...
  const handleChange = (checked: boolean, selectedItem: IDomainData) => {
    const recipientObj: IAvailableRecipient = data.availableRecipients.find(
      (recipient) => recipient.data.some((item) => item.id === selectedItem.id)
    );
      ...
      >
        {data.availableRecipients.map((recipient) => {
          if (recipient.data.length === 1) {
            ...
            return (
              <div key={id} style={singleRecipientContainer}>
                <Checkbox
                  isChecked={isSelected}
                  onChange={(e) =>
                    handleChange(e.target.checked, recipient.data[0])
                  }
                >
                  {email}
                </Checkbox>
              ...
              <AccordionItem key={recipient.id} border="none">
                <h2 style={accordionHeader}>
                  <Checkbox
                    ...
                    isIndeterminate={
                      recipient.data.some((item) => item.isSelected) &&
                      !recipient.data.every((item) => item.isSelected)
                    }
                    onChange={(e) =>
                      recipient.data.forEach((item) =>
