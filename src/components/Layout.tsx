...
import { recipientsRoot } from "./Layout.styled";
...
const Layout: FC = (): ReactElement => {
  ...
  const [alertDialogContent, setAlertDialogContent] =
    useState<IAlertDialogContent>();
  const toast = useToast();

  useEffect(() => {
    const availableRecipient: IAvailableRecipient[] = getAvailableRecipient(
      mockedData as IDomainData[]
    );
  ...
  const ctx = useMemo<ILayoutContextProps>(
    ...
    [state, dispatch, handleToast, handleAlertDialog]
  );
